...
import { Zap, Dice5, Shield, Sword, AlertTriangle, Layers, Settings2, Check, ArrowRight, Database, Eye, EyeOff, ChevronDown, ChevronRight, ExternalLink, Unlock, PlusCircle, Filter } from 'lucide-react';
...
const RandomizerPage: React.FC = () => {
  ...
  // 1. How many points earned? (Completed checklist items)
  const earnedPoints = useMemo(() => {
      return checklist.filter(i => i.isCompleted).length;
  }, [checklist]);

  // 2. How many points spent? (Number of unlocked skills minus the initial one)
  const spentPoints = useMemo(() => {
      if (!randomizedBuild?.unlockedSkills) return 0;
      // Initial build has 1 skill. Anything above that is "spent"
      return Math.max(0, randomizedBuild.unlockedSkills.length - 1);
  }, [randomizedBuild]);

  const availablePoints = Math.max(0, earnedPoints - spentPoints);
  
  // Determine if we are in "Add Skill Mode" (Settings ON + Existing Build)
  const isAddSkillMode = enableUnlockNewSkills && randomizedBuild !== null;


  // --- Data Filtering ---

  // Identify all tags that are technically available (not globally excluded)
  ...
  const basePool = useMemo(() => {
    return skills.filter(skill => 
        ...
        // Apply generator-specific filter: strictly check inclusion
  ...
  const handleRandomizeOptions = () => {
    ...
    if (!isAddSkillMode) {
        ...
        // In Add Skill mode, we need available points
    ...
    setTimeout(() => {
      // --- Randomize Classes (Skip if Add Skill Mode) ---
      ...
      // Determine source pool (basePool is already filtered by genSelectedTags)
      let poolToSampleFrom = [...basePool];

      // If adding skills to existing build, filter out duplicates
      if (isAddSkillMode && randomizedBuild) {
          const existingSkillNames = new Set((randomizedBuild.unlockedSkills || []).map(s => s.name));
          // Also ensure the main skill is covered (usually in unlockedSkills, but safety first)
          existingSkillNames.add(randomizedBuild.mainSkill.name);

          poolToSampleFrom = poolToSampleFrom.filter(skill => !existingSkillNames.has(skill.name));
      }
  ...
  const finalizeBuild = () => {
    if (isAddSkillMode) {
        ...
        if (selectedDraftSkill && randomizedBuild) {
            ...
            // Double check duplicate prevention before saving
            ...
            // Clear draft state so we don't accidentally add again
            setRandomizedSkills([]);
            setSelectedDraftSkill(null);
            // Remain on Randomizer Page (No navigate)
        }
    } else {
        // --- New Build Mode ---
        if (selectedDraftClass && selectedDraftSkill) {
            const finalBuild: RandomizedResult = {
                ...
                unlockedSkills: [selectedDraftSkill.main], // Initialize unlocked list
                timestamp: Date.now()
            };
            setRandomizedBuild(finalBuild);
            // Explicitly reset the checklist to default state (all incomplete) for the new build
            setChecklist(DEFAULT_CHECKLIST.map(item => ({...item, isCompleted: false})));
            
            // Clear randomizer results
            setRandomizedClasses([]);
            setRandomizedSkills([]);
            setSelectedDraftClass(null);
            setSelectedDraftSkill(null);
    ...
    <div className="space-y-8 pb-32 max-w-6xl mx-auto relative">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto">
          {isAddSkillMode 
            ...
            : 'Configure independent pool sizes, randomize variants, and mix & match your build.'}
        </p>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 shadow-lg max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Left Column: Class Config (Disabled in Add Mode) */}
      ...
      {/* Active Pool Visualization (Collapsible) */}
      ...
      {hasResults && (
        <div className="space-y-12 animate-in slide-in-from-bottom-8 duration-500 fade-in">
          ...
          {/* Block 1: Class Selection (Hidden in Add Skill Mode) */}
          {!isAddSkillMode && (
      ...
      {/* Sticky Bottom Bar for Build Construction */}
      {hasResults && (
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-950/90 backdrop-blur-lg border-t border-gray-800 z-40 animate-in slide-in-from-bottom-20">
              <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 md:gap-6 w-full md:w-auto">
                      ...
                      {/* Class Preview (Hidden in Add Mode) */}
                      {!isAddSkillMode && (
